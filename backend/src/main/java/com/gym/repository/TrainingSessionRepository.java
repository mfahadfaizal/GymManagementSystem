...
public interface TrainingSessionRepository extends JpaRepository<TrainingSession, Long> {
    ...
    List<TrainingSession> findSessionsByTrainerAndDateRange(@Param("trainer") User trainer, @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);
    
    @Query("SELECT ts FROM TrainingSession ts WHERE ts.member = :member AND ts.scheduledDate BETWEEN :startDate AND :endDate")
    List<TrainingSession> findSessionsByMemberAndDateRange(@Param("member") User member, @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);
